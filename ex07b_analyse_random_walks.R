...
library(tidyverse)
library(mcMST)
#load_all("~/repositories/software/mcMST")
...
res = read.table("results/random_walks/random_walks.csv", sep = " ", header = TRUE, stringsAsFactors = FALSE)
#res$doscal = ifelse(res$scalarize, "on", "off")
res$mut2 = res$mut
res$mut = ifelse(res$scalarize, sprintf("%sS", res$mut), res$mut)
res$mut = sprintf("%s (%i)", res$mut, res$max.sigma)
res$class = sapply(res$instance, substr, 1, 6)
res$class = gsub("LASS", "", res$class)
res$algorithm = res$mut
res = filter(res, repl <= 3)

instances = unique(res$instance)

pl = plotTraces(res)#, extreme.sols = extrsols))
ggsave("figures/random_walks/random_walks.pdf", plot = pl, width = 60, height = 50, limitsize = FALSE)
